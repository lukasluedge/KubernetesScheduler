...
public class CWSKubernetesClient implements AutoCloseable {
    ...
    // WICHTIG: static, damit Fabric8-Mock die Instanz injizieren kann
    ...
    private SharedIndexInformer<Pod> podInform;
    private SharedIndexInformer<Node> nodeInform;

    // Standard-Konstruktor (echter Cluster)
    ...
    // Injektion (Mock oder vorbereiteter Client)
    ...
    private void initInformers() {
        try {
            ...
            podInform = this.pods().inAnyNamespace().inform(new PodHandler(this));
            nodeInform = this.nodes().inform(new NodeHandler(this));
            log.info("Informers gestartet (Nodes initial: {})", existing.size());
        } catch (KubernetesClientException ex) {
            // Falls realer Cluster nicht erreichbar → klarere Fehlermeldung
            throw new IllegalStateException("Konnte Kubernetes nicht initialisieren. Läuft kein Cluster? "
                    + "Starte mit -Dcws.mock.enabled=true für den Mock. Ursache: " + ex.getMessage(), ex);
        }
    }
    @PreDestroy
    @Override
    public void close() throws Exception {
        if (podInform != null) podInform.close();
        if (nodeInform != null) nodeInform.close();
        ...
        for (int i = 0; i < 5; i++) {
            ...
            try {
                ...
                if (e.getStatus() != null &&
                        ...
                        e.getStatus().getMessage().toLowerCase().contains("is already assigned to node")) {
                    return; // ok
                }
                if (i == 4) throw e;
            }
    ...
    public boolean patchTaskMemory(Task t) {
        try {
            ...
            Resource<Pod> podResource = pods()
                    .inNamespace(namespace)
                    .withName(podname);
            Container container = podResource.get().getSpec().getContainers().get(0);
            ...
            if (e.toString().contains("Forbidden: pod updates may not change fields other than")) {
                log.error("Could not patch task. Ensure feature gate InPlacePodVerticalScaling is enabled.");
            } else {
    ...
    public Pod buildSimulatedPodForTask(Task task, Scheduler scheduler) {
        final TaskConfig cfg = task.getConfig();
        final String runName = cfg.getRunName();
        // Sicherheitsprüfungen
        if (runName == null || runName.isBlank()) {
            throw new IllegalArgumentException("runName must be set for simulated pod");
        }

        // CPU / Memory Requests (nur wenn > 0)
        Map<String, Quantity> requests = new HashMap<>();
        if (cfg.getCpus() > 0) {
            // CPU als „X“ (float) – fabric8 akzeptiert z.B. "1", "2", "500m".
            // Für einfache Simulation reicht Ganzzahl / float-String.
            requests.put("cpu", new Quantity(String.valueOf(cfg.getCpus())));
        }
        if (cfg.getMemoryInBytes() > 0) {
            // Memory in Bytes als nackte Zahl ist erlaubt; optional könntest du in Mi umrechnen.
            requests.put("memory", new Quantity(String.valueOf(cfg.getMemoryInBytes())));
        }

        ResourceRequirementsBuilder rr = new ResourceRequirementsBuilder();
        if (!requests.isEmpty()) {
            rr.addToRequests(requests);
        }

        Pod pod = new PodBuilder()
                .withNewMetadata()
                .withName(runName)
                .withNamespace(scheduler.getNamespace())
                .addToLabels("app", "cws")
                .addToLabels("runName", runName)
                .addToLabels("taskName", cfg.getTask())
                .addToLabels("processName", cfg.getTask())
                .addToLabels("simulated", "true")
                .endMetadata()
                .withNewSpec()
                .withSchedulerName(scheduler.getName()) // 'name' ist der Scheduler-Name im Code (prüfen)
                .addNewContainer()
                .withName("main")
                .withImage("busybox")
                .withCommand("sh", "-c", "echo simulated pod for " + runName + " && sleep infinity")
                .withResources(rr.build())
                .endContainer()
